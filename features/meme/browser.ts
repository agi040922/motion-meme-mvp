...
export const addCommentWithMedia = async (
  postId: string,
  content: string,
  imageFile: File | null,
) => {
  ...
  const normalizedMimeType = normalizeUploadMimeType(imageFile.type || "image/png");
  const objectPath = buildObjectPath(userId, `comment-${commentId}`, imageFile.name);
  ...
  try {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("post-media").getPublicUrl(objectPath);
    ...
    throw new Error(
      error instanceof Error ? error.message : "Comment image could not be uploaded.",
    );
  }
};
